...
package com.huawei.chitchatapp.videoplayer;
...
import com.huawei.chitchatapp.videoplayer.control.PlayControl;
...
import com.huawei.chitchatapp.videoplayer.utils.SelectDialog;
import com.huawei.chitchatapp.videoplayer.utils.StringUtil;
import com.huawei.chitchatapp.videoplayer.view.PlayView;
...
public class PlayActivity extends AppCompatActivity implements OnPlayWindowListener, OnWisePlayerListener {
    private static final String TAG = "Res PlayActivity";
    ...
    protected void onResume() {
        super.onResume();
        AppLog.logD(TAG, "onResume isSuspend:" + isSuspend);
        isResume = true;
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        AppLog.logD(TAG, "surface created");
    ...
    public void surfaceDestroyed(SurfaceHolder holder) {
        ...
        AppLog.logD(TAG, "surfaceDestroyed");
        isSuspend = true;
        playControl.suspend();
    }
    ...
    public void onLoadingUpdate(WisePlayer wisePlayer, final int percent) {
        AppLog.logD(TAG, "update buffering percent :" + percent);
        runOnUiThread(() -> {
            if (percent < 100) {
    ...
    public void onStartPlaying(WisePlayer wisePlayer) {
        AppLog.logD(TAG, "onStartPlaying");
        runOnUiThread(() -> {
            playView.dismissBufferingView();
    ...
    public boolean onError(WisePlayer wisePlayer, int what, int extra) {
        AppLog.logD(TAG, "onError what:" + what + " extra:" + extra);
        Toast.makeText(this, "error:" + what + " extra:" + extra, Toast.LENGTH_SHORT).show();

        runOnUiThread(() -> playView.dismissBufferingView());
    ...
    public void onSubtitleUpdate(WisePlayer wisePlayer, Parcelable[] subtitles) {
        AppLog.logD(TAG, "onSubtitleUpdate length:" + subtitles.length);
        if (subtitles.length > 0) {
            ...
            for (int i = 0; i < subtitles.length; i++) {
                if (subtitles[i] instanceof SubtitleInfo) {
                    ...
                    if (subtmp[i] != null) {
                        AppLog.logD(TAG, "onSubtitleUpdate subtitle:" + subtmp[i].getSubtitle() + ",startTS:" +
    ...
    public void onStartTrackingTouch(SeekBar seekBar) {
        ...
        AppLog.logD(TAG, "onStartTrackingTouch isUserTrackingTouch:" + isUserTrackingTouch);
    }

    @Override
    public void onStopTrackingTouch(SeekBar seekBar) {
        AppLog.logD(TAG, "onStopTrackingTouch Progress:" + seekBar.getProgress());
        isPlayComplete = false;
    ...
    private Handler updateViewHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg == null) {
                AppLog.logD(TAG, "current mss is empty.");
                return;
            }
            switch (msg.what) {
    ...
    private void setPlaySpeed() {
        try {
            ...
            AppLog.logD("current play speed : float is " + playControl.getPlaySpeed() ,", and String is" + speedValue);
            playView.showSettingDialogValue(Constants.PLAYER_SWITCH_PLAY_SPEED, Arrays.asList(showTextArray),
                speedValue);
        } catch (NotFoundException e) {
            AppLog.logD(TAG, "get string array error :" + e.getMessage());
        }
    }
    ...
    private void onSwitchRequestMode(String selectValue) {
        ...
        AppLog.logD(TAG, "mStreamRequestMode:" + streamRequestMode);
    ...
    private void onSwitchBitrate(String itemSelect) {
        if (itemSelect.equals(StringUtil.getStringFromResId(PlayActivity.this, R.string.automatic_adaptation))) {
            ...
            if (index != -1 && index <= playControl.getBitrateRangeList().size()) {
                ...
                AppLog.logD(TAG, "get bitrate error");
                return;
            }
            playView.showSwitchingBitrateTextView(itemSelect);
        }
    ...
    private void switchSubtitle() {
        if (this.wisePlayer != null) {
            ...
            if (infoList == null) {
                AppLog.logD(TAG, "switchSubtitle infoList == null");
                return;
            }
            if (infoList.length > 0) {
                List<String> showTextList = new ArrayList<>();
                AppLog.logD(TAG, "switchSubtitle infoList.length:" + infoList.length);
                for (SubtitleTrackInfo subtitleTrackInfo : infoList) {
                    String desc = subtitleTrackInfo.getDesc();
                    int id = subtitleTrackInfo.getId();
                    AppLog.logD(TAG, "switchSubtitle desc:" + desc + ", id:" + id);
                ...
                if (info != null) {
                    ...
                    AppLog.logD(TAG, "switchSubtitle selectedIndex:" + selectedIndex);
                }
    ...
    private void setVideoVolume() {
        DialogUtil.showSetVolumeDialog(this, inputText -> playControl.setVolume(StringUtil.valueOf(inputText)));
    }
    ...
    private void switchBitrateMenu(boolean isAuto) {
        ...
        AppLog.logD(TAG, "currentBitrate:" + currentBitrate);
        if (currentBitrate == 0) {
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
        AppLog.logD(TAG, "onSurfaceTextureAvailable");
        hasSurfaceCreated = true;
    ...
    public boolean onSurfaceTextureDestroyed(SurfaceTexture surface) {
        AppLog.logD(TAG, "onSurfaceTextureDestroyed");
        hasSurfaceCreated = false;
        isSuspend = true;
        playControl.suspend();
        return false;
    ...
    public void onItemClick(int pos) {
        AppLog.logD(TAG, "select new video url");
        if (!isPlayComplete) {
            playControl.savePlayProgress();
            isPlayComplete = false;
        }
        ...
        switch (settingType) {
            case Constants.MSG_SETTING:
                if (TextUtils.equals(itemSelect,
                    ...
                    AppLog.logD(TAG, "current settings type is " + itemSelect);
                }
                break;
            ...
            case Constants.PLAYER_GET_AUDIO_TRACKS:
                if (TextUtils.equals(itemSelect,
                    ...
                    AppLog.logD(TAG, "getSelectedAudioTrack is :" + itemSelect);
                    playControl.getSelectedAudioTrack();
                } else {
                    AppLog.logD(TAG, "getAudioTracks is :" + itemSelect);
    ...
    public boolean onEvent(WisePlayer wisePlayer, int what, int extra, Object o) {
        AppLog.logD(TAG, "onEvent = " + what + " extra = " + extra);
    ...
    public void onPlayEnd(WisePlayer wisePlayer) {
        AppLog.logD(TAG, "onPlayEnd " + wisePlayer.getCurrentTime());
    ...
    public void onResolutionUpdated(WisePlayer wisePlayer, int w, int h) {
        AppLog.logD(TAG, "current video width:" + w + " height:" + h);
        playView.setContentView(wisePlayer, playControl.getCurrentPlayName());
    }
    ...
    public void getSwitchAudioTrack() {
        ...
        if (audioTrack == null) {
            AppLog.logD(TAG, "switchAudio no audio track");
        ...
        for (AudioTrackInfo audioTrackInfo : audioTrack) {
            showTextList.add(audioTrackInfo.getDesc());
        }
        AppLog.logD(TAG, "getSwitchAudioTrack current audiotrack:" + playControl.getAudioLangIndex());
